 ...
 * Based on OpenEvals JSON export structure.
 */
 ...
 * Converts OpenEvals format to Geval's normalized format.
 *
...
export class OpenEvalsAdapter implements EvalAdapter {
  ...
  /**
   * Check if the data looks like OpenEvals output
   */
  supports(data: unknown): boolean {
    if (!isObject(data)) return false;

    // OpenEvals typically has 'results' array with 'scores' or 'passed'
    const hasResults =
      "results" in data &&
    ...
    // Or has summary with pass/fail counts
    const hasSummary =
      "summary" in data &&
      isObject(data.summary) &&
      ("passed" in data.summary || "accuracy" in data.summary);

    // Or has pre-computed metrics
    const hasMetrics =
      "metrics" in data && isObject(data.metrics);

    return hasResults || hasSummary || hasMetrics;
  }

  /**
   * Parse OpenEvals output and convert to normalized format
   */
  ...
  /**
   * Convert OpenEvals result to normalized format
   */
  private normalize(data: OpenEvalsResult): NormalizedEvalResult {
    ...
    if (data.summary) {
      ...
      // Calculate pass rate if we have the data
      if (
        data.summary.total !== undefined &&
        data.summary.passed !== undefined &&
    ...
    return {
      ...
      runId: data.eval_id ?? generateRunId(),
